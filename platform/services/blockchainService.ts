...
export interface IssueBrandBadgeInput {
  brandWallet: string;
  brandId: string;
  metadataURI: string;
}

export interface IssueBrandBadgeResult {
  txHash: string;
  tokenId: string;
  contractAddress: string;
}

export interface RevokeBrandBadgeResult {
  txHash: string;
  contractAddress: string;
}
...
const REWARD_PROOF_ABI = [
  ...
  "function computeProofHash(string userId,string matchId,string rewardType,uint256 timestamp) pure returns (bytes32)",
];
...
const BRAND_VERIFICATION_ABI = [
  "function issueBadge(address brandWallet,string brandId,string metadataURI) returns (uint256)",
  "function revokeBadge(uint256 tokenId)",
  "function isVerified(address brandWallet) view returns (bool)",
];
...
async function getContracts(): Promise<ContractsBundle> {
  ...
  contractsCache = {
    ...
    brandVerification: new Contract(
      requiredEnv("BRAND_VERIFICATION_BADGE_ADDRESS"),
      BRAND_VERIFICATION_ABI,
      signer
    ),
  };
...
export async function issueBrandBadgeOnChain(
  input: IssueBrandBadgeInput
): Promise<IssueBrandBadgeResult> {
  const contracts = await getContracts();

  const tokenId = await contracts.brandVerification.issueBadge.staticCall(
    input.brandWallet,
    input.brandId,
    input.metadataURI
  );

  const tx = await contracts.brandVerification.issueBadge(
    input.brandWallet,
    input.brandId,
    input.metadataURI
  );
  ...
  return {
    txHash: tx.hash,
    tokenId: tokenId.toString(),
    contractAddress: contracts.brandVerification.target.toString(),
  };
}

export async function revokeBrandBadgeOnChain(tokenId: string): Promise<RevokeBrandBadgeResult> {
  const contracts = await getContracts();

  const tx = await contracts.brandVerification.revokeBadge(BigInt(tokenId));
  await tx.wait();
...
export async function isBrandVerifiedOnChain(brandWallet: string): Promise<boolean> {
  const contracts = await getContracts();
  return contracts.brandVerification.isVerified(brandWallet);
}
